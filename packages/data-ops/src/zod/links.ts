...
export const cloudflareInfoSchema = z.object({
    ...
    latitude: z.string().transform((val) => (val ? Number(val) : undefined)).optional(),
    longitude: z.string().transform((val) => (val ? Number(val) : undefined)).optional(),
});
...
export const durableObjectGeoClickSchemaType = z.array(durableObjectGeoClickSchema);

export type DurableObjectGeoClickSchemaType = z.infer<typeof durableObjectGeoClickSchema>;

export type CloudflareInfoSchemaType = z.infer<typeof linkSchema>;
